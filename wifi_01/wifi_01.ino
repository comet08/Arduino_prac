 ...
 * Setup your serial monitor with "Both NL & CR" 
 ...
 AT+CWMODE?   :  check the mode of esp8266 (1 : station or client, 2 : AP or Host, 3 : Station/AP Dual) 
 AT+CWMODE=1  :  set the mode of esp8266 to client 
 AT+CWMODE=2  :  set the mode of esp8266 to ap 
 AT+CWLAP     :  list available APs  (output e.g. : +CWLAP:(ecn,ssid,rssi,mac,ch,freq offset,freq calibration), ecn:0=open,1=wep,2=wpa_psk,3=wpa2_psk,4=wpa_wpa2_psk) 
 AT+CWJAP="SSID of AP","Password"  : connect to 
 AP AT+CWJAP?   : check the connected AP 
 ...
 AT+CIPSEND=4,18  : Send command with length 18 on the connection "4"  (use this command  "GET / HTTP/1.1" ) 
